...
void kvmmapuser(pagetable_t kpagetable, pagetable_t upagetable,
                uint64 newsz, uint64 oldsize)
{
  uint64 va;
  pte_t *upte;
  pte_t *kpte;
  ...
  if (newsz >= PLIC)
  {
    panic("kvmmapuser: newsz too big");
  }

  for (va = oldsize; va < newsz; va += PGSIZE)
  {
    upte = walk(upagetable, va, 0);
    if (upte == 0 || (*upte & PTE_V) == 0)
    {
      panic("kvmmapuser: upte not present");
    }

    kpte = walk(kpagetable, va, 1);
    if (kpte == 0)
    {
      panic("kvmmapuser: kpte not present");
    }
...
void vmprinthelper(pagetable_t pagetable, int space)
{
  for (int i = 0; i < 512; i++)
  {
    pte_t pte = pagetable[i];
    ...
    if ((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0)
    {
      ...
      vmprinthelper((pagetable_t)child, ++space);
      space--;
    }
  }
}
